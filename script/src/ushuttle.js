...
function setPeriod(state) {
    ...
    if (isZH) {
        ...
        if ((state === 1 && isSatAvailable === 0) || state === 2 || state === 3) {
...
$(document).ready(function(){
    ...
    $.get("../../json/cycle.json", function(xhr) {
        if(xhr.cycle !== undefined && xhr.cycle !== null) {
            ...
            tdata_to = fetchTimetable("tocampus", jsondata.timetable.weekday.tocampus);            
            tdatasat_to = fetchTimetable("tocampus", jsondata.timetable.saturday.tocampus);            
            tdata_from = fetchTimetable("fromcampus", jsondata.timetable.weekday.fromcampus);
            tdatasat_from = fetchTimetable("fromcampus", jsondata.timetable.saturday.fromcampus);

            loadTimetable(tdata_to, ".timetable.normal.to");
            loadTimetable(tdatasat_to, ".timetable.sat.to");
            loadTimetable(tdata_from, ".timetable.normal.from");
            loadTimetable(tdatasat_from, ".timetable.sat.from");
            ...
            if (daystate === 0) {
                ...
                eta_to = tdatasat_to;
                eta_from = tdatasat_from;        
            }
            if(daystate === 0 || daystate === 1) {
                getNextBusTime(eta_to, ".nextbus.tocampus");
                getNextBusTime(eta_from, ".nextbus.fromcampus");
            }
